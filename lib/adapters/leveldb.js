...
function LevelPouch(opts, callback) {
  ...
  api._info = function (callback) {
    ...
    incrementDocCount(0, function (err, docCount) {
      // TODO: handle err
  ...
  function incrementDocCount(delta, callback) {
    ...
    function applyNextDelta() {
      ...
      stores.bySeqStore.get(DOC_COUNT_KEY, function (err, docCount) {
        if (err) { // TODO: handle other errors besides 404
          docCount = 0;
        }

        function complete() {
          item.callback(null, docCount);
          queue.running = false;
          applyNextDelta();
        }
    ...
    queue.queue.push({delta : delta, callback : callback});
    applyNextDelta();
  }
  ...
  api._bulkDocs = function (req, opts, callback) {
    ...
    function insertDoc(doc, callback) {
      ...
      writeDoc(doc, function () {
        ...
        incrementDocCount(1, function (err) {
          if (err) {
            // TODO: handle error
          }
          return callback();
  ...
  api._allDocs = function (opts, callback) {
    // cheap way to ensure we get the freshest docCount
    incrementDocCount(0, function (err, docCount) {
      // TODO: handle err
      var readstreamOpts = {};
